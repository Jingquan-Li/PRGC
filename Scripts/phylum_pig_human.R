df<- read.table("Human_phylum_relative.tab",row.names =1,sep="\t",quote="",check.names=F)
...
###Pig phylum
phy<- read.table("Pig_phylum_relative.tab",row.names =1,sep="\t",quote="",check.names=F)
...
human_pig$Taxonomy <-  factor(human_pig$Taxonomy,levels=c("Actinobacteria","Apicomplexa", "Ascomycota", "Bacteroidota" ,"Cossaviricota", "Deinococcus-Thermus","Firmicutes",  "Fusobacteria" ,"Nematoda","Peploviricota"  ,     "Proteobacteria","Spirochaetes",        "Tenericutes" ,        "Uroviricota","Others"))
...
ggbarplot(human_pig, x = "variable", y="Abundance", size=0.2,color="Phylum",fill="Phylum",legend="right",
	...
	scale_color_manual(values=c(colorRampPalette(brewer.pal(8,'Accent'))(16)[1:10],"#56A0CC",brewer.pal(8,'Dark2')[1:3],"gray"))+
	scale_fill_manual(values=c(colorRampPalette(brewer.pal(8,'Accent'))(16)[1:10],"#56A0CC",brewer.pal(8,'Dark2')[1:3],"gray"))+
